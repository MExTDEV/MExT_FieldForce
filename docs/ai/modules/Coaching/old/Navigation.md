...
- `Rollen` and `Modules` are shown as sub-items under `Instellingen` in the
  Beheer sidebar.
- Their existing routes and permission keys remain unchanged:
  `/beheer/rollen` still requires `menu.coaching.roles` and `/beheer/modules`
  still requires `menu.coaching.modules`.
- This grouping is visual only and must not broaden access to role or module
  management.
