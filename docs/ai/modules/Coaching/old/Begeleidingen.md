...
## Criteria and score questions

Begeleidingen use a criteria snapshot.

At creation/planning time, applicable KPI, Kapstok, general evaluation and personality criteria are resolved from the coached user's country, team and user scope. The resulting snapshot is stored on the coaching and is used for later loading and reporting. Configuration changes after creation do not alter existing coachings.

Scope grouping order is:
...
If one criterion is linked through multiple applicable scopes, it is shown once under the most specific applicable scope and uses that scope link's sort order.
...
- read-only
- empty remarks remain visually empty; the dossier must not show placeholder text such as `Geen opmerking ingevuld.`
- whitespace-only text and empty rich-text HTML such as `<p><br></p>` are treated as empty remarks
- when a coaching was executed by a Professional or Expert representative, show
  clearly: `Uitgevoerd door collega-vertegenwoordiger: [naam] — [Professional/Expert]`.
- if the coaching has a stored team/country deviation, the reason is visible to
  planner, executor and authorised audit users, but not by default to the
  coached representative.
...
- A Professional/Expert representative may open and execute only the coachings
  where that representative is explicitly assigned as owner/executor.
- A Professional/Expert representative may not plan, reschedule, cancel or change
  the planned date/time from Begeleidingen.
