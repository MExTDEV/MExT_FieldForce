...
## Criterion snapshot rule

When a coaching is created or planned, FieldForce creates a criteria snapshot for the coached user.

The snapshot is based on:

- current country of the coached user;
- current team of the coached user;
- personal user-specific criterion links;
- all active global criterion links.

Planner, executor, manager and the user filling in the form do not influence criterion selection.

Applicable criteria are cumulative across Global, Country, Team and User. When the same criterion applies through multiple links, it appears once under the most specific applicable scope:
...
Existing coachings never recalculate their criteria when configuration changes later.
...
- `Hulpaanvragen.md` describes the representative-to-manager help request workflow.
...
- The planning wizard supports the original representative notification checkbox.
- For the Professional/Expert peer-coaching flow it also stores the three
  explicit notification choices:
  - coached representative beforehand;
  - sales leader(s) of the coached representative;
  - sales leader(s) of the executing Professional/Expert.
- All three choices are off by default for new coachings.
- The executing Professional/Expert is always functionally informed; no checkbox
  is needed for that mandatory notification.
...
### Professional/Expert as executor

A coaching remains a normal coaching. It must not become a separate coaching
type. An active representative with level Professional or Expert can be selected
as executor for another representative when the planner is Group Manager, Sales
Manager, Country Manager, Admin or Super Admin.

Rules:

- executor and coached representative must be different users;
- Starter and Sales Executive representatives cannot execute peer coachings;
- Professional/Expert representatives cannot create, plan, reschedule, cancel or
  change date/time themselves;
- cross-team or cross-country planning is allowed only with a required
  deviation reason;
- the deviation reason is visible to planner, executor and auditors, not to the
  coached representative by default;
- cross-scope selection grants access only to that coaching record and never to
  broader user fiches, scores, action points or modules.
...
Extended statuses and flags introduced for Professional/Expert peer coaching:

- `Niet uitgevoerd` after the planned day passes without an actual start;
- `Te laat af te werken` as a deadline flag after two working days;
- `Actiepunten te beoordelen` as review phase before the coached person sees
  action points;
- `Wacht op finale goedkeuring`, `Finaal goedgekeurd` and `Finaal afgewezen` as
  manager-controlled final review semantics;
- `Geannuleerd` before start and `Administratief afgesloten` after start.

The technical model supports these through added status and metadata fields. Jobs
and complete UI actions for all transitions must remain idempotent and
server-side validated.

---

# Related Flow: Hulpaanvragen

Hulpaanvragen are part of Coaching support, but they are not a duplicate Coaching form.

Rules:

- only a `Vertegenwoordiger` can create a help request, and only for themselves;
- the responsible manager is resolved server-side from team and scope data;
- the representative can edit or withdraw only while the request is untreated;
- the manager can answer, answer and close, or select one primary follow-up;
- manager answers are one-way handling notes, not a chat thread;
- selecting follow-up reuses the existing Begeleiding, Contactmoment, Retraining or Salestraining workflow.
- selecting `Begeleiding` opens the existing planning wizard and only creates a real planned coaching after confirmation.

See `Hulpaanvragen.md` for statuses, persistence and validation.
