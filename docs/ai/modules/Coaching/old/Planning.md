...
- contact moments use the shared workflow state and route to
  `/contactmomenten/:id`
- Planning uses the stored contact moment date and time instead of falling back
  to the creation date
- representative visibility follows the contact moment notification/share rule:
  hidden before share when `Vooraf op de hoogte brengen` is off, visible when
  announced, and visible after final sharing
...
- optional Professional/Expert executor metadata
- optional team/country deviation flags and required deviation reason
- optional pre-notification choices for coached representative and team leaders

The Planning item must link back to the existing coaching record.

Professional/Expert representatives may see and start only the coachings for
which they were explicitly assigned as executor. They may not create, reschedule,
cancel or change the date/time from Planning. Cross-team or cross-country
metadata grants access only to that specific coaching record.
...
- help request items open the existing Hulpaanvragen detail flow;
- if a manager chooses `Begeleiding` as follow-up, the existing Begeleiding
  planning wizard is opened and the actual coaching is created only after
  wizard confirmation;
- Planning keeps showing the resulting real coaching through the normal
  Begeleiding calendar item once it exists.
